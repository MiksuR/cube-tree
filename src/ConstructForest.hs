...
Through out the construction we keep track of the cubies we have already
inserted and the remaining cubies at our disposal.
...
-}
module ConstructForest (Layer, cubeForest, cubeMiniTree) where
...
-- |This is an analogous object for the 2×2 cube.
cubeMiniTree :: Tree Layer
cubeMiniTree = unfoldTree buildMiniNode
...
buildNode (layer, edgs, corns, n) = (layer, filter (valid layer) embedded)
  where
    ...
    embedLayer cs =
      ([ CubieEmbed { cubie = c, location = cubieLocation (n+1) i } | (c, i) <- zip cs [0..] ],
       if isCorners then edgs else difference edgs (fromList . map orient $ cs),
       if isCorners then difference corns (fromList . map orient $ cs) else corns, n+1)
...
buildMiniNode (layer, _, corns, n) = (layer, filter (validMini layer) embedded)
  where
    ...
    embedLayer :: [Cubie] -> LayerSeed
    embedLayer cs =
      ([ CubieEmbed { cubie = c, location = cubieLocation (n+2) i } | (c, i) <- zip cs [0..] ],
       empty, difference corns (fromList . map orient $ cs), n+2)
...
combinations isCorners n s = foldl' adjoiner Empty s
  where
    adjoiner l a = l >< join (applyRotations a <$> combinations isCorners (n-1) (delete a s))
...
validMini ol    (nl,  _, _, 4) =
  and $ [ not $ shareColor (ol !! i) (nl !! mod (i+1) 2) | i <- [0..2] ]
